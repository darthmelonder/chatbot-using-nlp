...
appnames = set()
os.system("dpkg --get-selections | grep -v deinstall > ~/Minor/packages_rough")
os.system("cut -f 1 packages_rough>packages")
os.system("rm packages_rough")

f = open('packages','r')
for line in f:
	appnames.add(line.strip())
...
def main():
	while 1 == 1 :
		...
		for i in range (0,len(tokens)):
			if tagged_arr[i+1] in ['NN','NNS','NP','VB'] and tokens[i]!= 'open':
				for j in range(0,len(appnames)):
					if tokens[i] in appnames[j]:
						tagged_arr[i+1]='FN'
		...
		grammar_string = """
		  S -> NP VP
		  S -> VP
		  NP -> MODAL PRON | DET NP | NOUN VF | NOUN | FILENAME
 		  MODAL -> MD
      		  PRON -> PPSS | PPO
 		  VP -> VERB NP
		  VP -> VERB VP
		...
		for tree in parser.parse(tokens):
			json_str.append(dict_to_json(tree2dict(tree)))
